...
from modules.admission.constants import AdmissionStatus, Qualification, BloodGroup, Gender, ADMISSION_STATUS_COLORS
...
class AdmissionFormModal(ft.AlertDialog):
    """
    Enterprise Comprehensive Admission Form Modal.
    Structured in clear visual sections:
    1. Personal Information (Dual enrollment: Existing Search OR Inline Registration)
    2. Location (Village / Address - At least one required)
    3. Village / Friend Suggestions (Max 3 selectable)
    4. Academic Qualification & School/College Master
    5. Course & Dynamic Batch Selection
    6. Documents (Photo & Signature <= 100 KB)
    7. Financial Calculation & Confirmation Payment
    """
    ...
    def __init__(
        ...
        self.enrollment_mode: str = "existing" if (not self.is_edit_mode or admission) else "new"
        ...
        self.student_search_results = ft.Column(spacing=2, scroll=ft.ScrollMode.AUTO, height=120)
        self.selected_student_badge = ft.Container(visible=bool(self.selected_student_id))

        # ── Personal Information Fields ──
        self.first_name_input = ft.TextField(label="First Name *", hint_text="e.g. Rahul", border_radius=AppTheme.RADIUS_MD, value=admission.first_name if admission else "")
        self.middle_name_input = ft.TextField(label="Middle Name *", hint_text="Father's / Husband's name", border_radius=AppTheme.RADIUS_MD, value=admission.middle_name if admission else "")
        self.last_name_input = ft.TextField(label="Surname / Last Name *", hint_text="e.g. Sharma", border_radius=AppTheme.RADIUS_MD, value=admission.last_name if admission else "")
        self.mother_name_input = ft.TextField(label="Mother's Name *", hint_text="e.g. Sunita", border_radius=AppTheme.RADIUS_MD, value=admission.mother_name if admission else "")
        self.dob_input = ft.TextField(label="Date of Birth (YYYY-MM-DD) *", hint_text="2005-08-15", border_radius=AppTheme.RADIUS_MD, value=admission.dob if admission else "2006-01-01")
        self.gender_dropdown = ft.Dropdown(
            label="Gender *",
            options=[ft.DropdownOption(key="MALE", text="Male"), ft.DropdownOption(key="FEMALE", text="Female"), ft.DropdownOption(key="OTHER", text="Other")],
            value=admission.gender.upper() if (admission and admission.gender) else "MALE",
            border_radius=AppTheme.RADIUS_MD,
            on_select=lambda _: self._load_friend_suggestions(),
        )
        self.mobile_input = ft.TextField(label="Mobile Number *", hint_text="10-digit mobile number", keyboard_type=ft.KeyboardType.PHONE, border_radius=AppTheme.RADIUS_MD, value=admission.mobile_number if admission else "")
        self.email_input = ft.TextField(label="Email Address (Optional)", hint_text="student@example.com", keyboard_type=ft.KeyboardType.EMAIL, border_radius=AppTheme.RADIUS_MD, value=admission.email if admission else "")
        self.aadhaar_input = ft.TextField(label="Aadhaar Number *", hint_text="12-digit Aadhaar", keyboard_type=ft.KeyboardType.NUMBER, border_radius=AppTheme.RADIUS_MD, value=admission.aadhaar_number if admission else "")
        self.parent_name_input = ft.TextField(label="Parent / Guardian Name (Optional)", hint_text="e.g. Ravindra Sharma", border_radius=AppTheme.RADIUS_MD, value=admission.parent_guardian_name if admission else "")
        ...
        self.village_input = ft.TextField(
            ...
            on_change=lambda _: self._load_friend_suggestions(),
        )
        self.address_input = ft.TextField(
            label="Full Address (Optional if Village provided)",
            hint_text="Street, House No, Landmark...",
            border_radius=AppTheme.RADIUS_MD,
        ...
        self.friends_card = ft.Container(
            content=ft.Column(
                controls=[
                    ft.Text("Suggested Friends from this Village (Select up to 3):", size=AppTheme.SIZE_CAPTION, weight=ft.FontWeight.BOLD, color=AppTheme.TEXT_SECONDARY),
    ...
    def _build_content_layout(self) -> None:
        personal_section = ft.Container(
            content=ft.Column(
                controls=[
                    ft.Text("1. Personal Information", weight=ft.FontWeight.BOLD, size=AppTheme.SIZE_BODY, color=AppTheme.PRIMARY),
                    self.mode_segmented_btn,
                    self.student_search_input,
                    self.student_search_results,
                    self.selected_student_badge,
                    ft.Row([self.first_name_input, self.middle_name_input, self.last_name_input]),
                    ft.Row([self.mother_name_input, self.parent_name_input]),
                    ft.Row([self.dob_input, self.gender_dropdown, self.blood_group_dropdown]),
                    ft.Row([self.mobile_input, self.email_input, self.aadhaar_input]),
                ],
                spacing=AppTheme.PAD_SM,
            ),
        ...
        location_section = ft.Container(
            content=ft.Column(
                controls=[
                    ft.Text("2. Location & Friend Matching", weight=ft.FontWeight.BOLD, size=AppTheme.SIZE_BODY, color=AppTheme.PRIMARY),
                    ft.Row([self.village_input, self.address_input]),
                    self.friends_card,
                ],
                spacing=AppTheme.PAD_SM,
            ),
        ...
        academic_section = ft.Container(
            content=ft.Column(
                controls=[
                    ft.Text("3. Academic & Course Enrollment", weight=ft.FontWeight.BOLD, size=AppTheme.SIZE_BODY, color=AppTheme.PRIMARY),
                    ft.Row([self.qualification_dropdown, self.qual_other_input, self.institution_dropdown]),
                    ft.Row([self.course_dropdown, self.batch_dropdown]),
                ],
                spacing=AppTheme.PAD_SM,
            ),
            bgcolor=AppTheme.SURFACE,
        ...
        fee_section = ft.Container(
            content=ft.Row(
                controls=[
                    ft.Row([ft.Text("Course Fee:"), self.base_fee_text]),
                    self.discount_input,
                    ft.Row([ft.Text("Final Fee:"), self.final_fee_text]),
                ],
    ...
    def _update_batches_for_course(self, course_id: int) -> None:
        batches = self.batch_controller.list_batches(course_id=course_id, status="OPEN")
    ...
    def _load_friend_suggestions(self) -> None:
        ...
        if not suggestions:
            self.friends_container.controls = [ft.Text("No recent students from this village.", italic=True, size=AppTheme.SIZE_CAPTION, color=AppTheme.TEXT_SECONDARY)]
        else:
            chips = []
            for f in suggestions:
                ...
                chips.append(
                    ft.Checkbox(
                        label=f"{f.display_name} ({f.gender or 'N/A'}, {f.course_name or 'Course'})",
                        value=is_selected,
    ...
    def handle_confirm_flow(self, e: ft.ControlEvent) -> None:
        """Saves admission in REGISTERED state and opens Payment Dialog to collect confirmation payment >= ₹500."""
        self.error_container.visible = False
        try:
            payload = self._gather_form_payload(AdmissionStatus.REGISTERED)
            # Create the initial admission in Registered state
            adm_id = self.controller.create_admission(payload)
            ...
            if self.page:
                ...
                self.page.show_dialog(pay_dialog)
    ...
    def _open_receipt_dialog(self, admission_id: int) -> None:
        try:
            ...
            if receipts and self.page:
                ...
                self.page.show_dialog(r_dialog)
        except Exception as ex:
